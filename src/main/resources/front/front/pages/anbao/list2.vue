	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"var(--publicSubColor)","color":"#fff","textAlign":"left","borderRadius":"20rpx 0","borderWidth":"0 10rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.anbaoName" type="text" placeholder="安保姓名"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('anbao','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
	<view style="">
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.anbaoName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('anbao','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                 <view v-show="item.anbaoPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">安保手机号</text>
                     <text style="width: 70%">{{item.anbaoPhone}}</text>
                 </view>
                 <view v-show="item.anbaoIdNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">安保身份证号</text>
                     <text style="width: 70%">{{item.anbaoIdNumber}}</text>
                 </view>
            ...
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 12rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('anbao','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
            </view>
        </view>
    </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`anbao`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('anbao', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.anbaoName) {
                    searchForm['anbaoName'] = this.searchForm.anbaoName
                }
				if (this.searchForm.anbaoPhone) {
                    searchForm['anbaoPhone'] = this.searchForm.anbaoPhone
                }
				if (this.searchForm.anbaoIdNumber) {
                    searchForm['anbaoIdNumber'] = this.searchForm.anbaoIdNumber
                }

                let res = await this.$api.list(`anbao`, searchForm);
				// 如果是第一页数据置空
