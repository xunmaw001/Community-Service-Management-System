	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('fankiu','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.fankiuName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('fankiu','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
            ...
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 12rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('fankiu','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
            </view>
        </view>
    </view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            this.categoryList = fankiuTypes.data.list

	   		//当前表的 反馈状态 字段 字典表查询方法
		  	let fankiuZhuangtaiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'fankiu_zhuangtai_types'
		  	});
		  	this.fankiuZhuangtaiTypesList = fankiuZhuangtaiTypes.data.list;
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.fankiuTypes = this.categoryName
                            }
