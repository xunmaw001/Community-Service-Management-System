	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('fankiu', this.fankiuId);
                    this.detail = res.data
					this.detail.fankiuContent = this.detail.fankiuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.fankiuPhoto ? this.detail.fankiuPhoto.split(",") : [];
				},
